...
function HomePage(props) {
  ...
  const [form, SetForm] = useState({
    codigoUsuario: props.usuario,
    codigoUnidade: props.unidade,
    senha: "",
  });
    ...
    try {
      const fetchUsuarios = async () => {
        const response = await axios.get(
          "https://ironrest.herokuapp.com/mUsuarios"
        );
  ...
  const lerUnidades = () => {
    try {
      const fetchUnidades = async () => {
        const response = await axios.get(
          "https://ironrest.herokuapp.com/mUnidades"
        );
  ...
  const handleSubmit = (e) => {
    ...
    if (
      mUsuarios.some(
        (c) => (c.codigo === form.codigoUsuario) & (c.senha === form.senha)
      ) & mUnidades.some((c) => c.codigo === form.codigoUnidade)
    ) {
      props.validaWork(
        form.codigoUsuario,
        form.codigoUnidade,
        mUnidades[mUnidades.findIndex((c) => c.codigo === form.codigoUnidade)]
          .descricao,
        mUsuarios[mUsuarios.findIndex((c) => c.codigo === form.codigoUsuario)]
          .permissao
      );
    } else {
